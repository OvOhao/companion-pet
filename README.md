...
除了插件自动出场，本项目还带一个 **Claude Code Skill**，让你直接用大白话管理桌宠：

> 「帮我启动桌宠」「把这张图加进桌宠」「我的桌宠有哪些角色」「换个角色」「改一下它说的话」

安装（让 Claude 能识别）：
```bash
mkdir -p ~/.claude/skills/companion-pet
cp -r skills/companion-pet/* ~/.claude/skills/companion-pet/
```
之后在 Claude Code 里说一句「启动我的桌宠」就会触发啦~

> 插件（hooks）负责**自动出场**，Skill 负责**对话控制**，两者共用同一套 CLI，互补使用。
